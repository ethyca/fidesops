...
from fidesops.models.email import EmailConfig, EmailConfigPurpose
...
def _get_email_config_purposes_by_key(db: Session, key: FidesKey) -> List[str]:
    return [item.purpose for item in db.query(EmailConfigPurpose).filter_by(config_key=key)]
...
def create_email_config(db: Session, config: EmailConfigRequest) -> EmailConfigResponse:
    ...
    for purpose in config.purposes:
        email_purpose: EmailConfigPurpose = EmailConfigPurpose.create_or_update(
            db=db,
            data={
                EmailConfigPurpose(config_key=config.key, purpose=purpose).dict()
            }
        )
        purposes.append(email_purpose)
    return EmailConfigResponse(
        name=email_config.name,
        key=email_config.key,
        service_type=email_config.service_type,
        details=email_config.details,
        purposes=[item.purpose for item in purposes]
    )
...
def delete_email_config(db: Session, key: FidesKey) -> None:
    ...
    purposes: List[EmailConfigPurpose] = db.query(EmailConfigPurpose).filter_by(config_key=key)
...
def get_email_configs(db: Session) -> Optional[List[EmailConfigResponse]]:
    ...
    for config in email_configs:
        email_configs_response.append(EmailConfigResponse(
            ...
            service_type=config.service_type,
            details=config.details,
            purposes=_get_email_config_purposes_by_key(db, config.key)
        ))
...
def get_email_config_by_key(db: Session, key: FidesKey) -> Optional[EmailConfigResponse]:
    ...
    return EmailConfigResponse(
        ...
        service_type=config.service_type,
        details=config.details,
        purposes=_get_email_config_purposes_by_key(db, key)
    )
