        ...
        for param in current_request.request_params:
            if param.type == "query":
                ...
                if param.default_value:
                    body = body.replace(f"<{param.name}>", f'"{param.default_value}"')
                elif param.references or param.identity:
                    body = body.replace(
                        f"<{param.name}>", f'"{input_data[param.name][0]}"'
                    )
                else:
            ...
            if param.type == "query":
                ...
                if param.default_value:
                    custom_body = custom_body.replace(
                        f"<{param.name}>", f'"{param.default_value}"'
                    )
                elif param.references:
                    custom_body = custom_body.replace(
                        f"<{param.name}>",
                        f'"{pydash.get(param_values, param.references[0].field)}"',
                    )
                elif param.identity:
                    custom_body = custom_body.replace(
                        f"<{param.name}>",
                        f'"{pydash.get(param_values, param.identity)}"',
                    )
                else:
                    logger.info(f"Missing body param value(s) for {path}")
