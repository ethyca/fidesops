...
async def verify_oauth_client(
    ...
    if not issued_at:
        raise AuthorizationError(detail="Not Authorized for this action 1")

    if is_token_expired(
        datetime.fromisoformat(issued_at),
        config.security.OAUTH_ACCESS_TOKEN_EXPIRE_MINUTES,
    ):
        raise AuthorizationError(detail="Not Authorized for this action 2")

    assigned_scopes = token_data[JWE_PAYLOAD_SCOPES]
    if not set(security_scopes.scopes).issubset(assigned_scopes):
        raise AuthorizationError(detail="Not Authorized for this action 3")
    ...
    if not set(assigned_scopes).issubset(set(client.scopes)):
        ...
        raise AuthorizationError(detail="Not Authorized for this action 6")
    return client
