...
query = text("""select datasetconfig.id, dataset from datasetconfig, connectionconfig where connectionconfig.id = datasetconfig.connection_config_id and connectionconfig.connection_type = 'saas';
    """)
update_query = text("""update datasetconfig set dataset = jsonb_set(dataset, '{type}', :saas_type) where id = :id""")
...
def upgrade():
    ...
    for id, dataset in connection.execute(query):
        ...
        try:
            saas_type: str = next((opt for opt in saas_options if any([fides_key.lower().startswith(opt), saas_name.lower().startswith(opt)])), "custom")
            connection.execute(update_query, {"saas_type": f"\"{saas_type}\"", "id": id})
        except Exception:
            # default to using "custom"
