...
def get_all_connection_types(
    *, params: Params = Depends(), search: Optional[str] = None
    ...
    """Returns a list of connection options in Fidesops - includes both database and saas options."""
    ...
    database_types: List[str] = [
        ...
        if conn_type not in [ConnectionType.saas, ConnectionType.https]
        and is_match(conn_type.value)
    ]
    saas_types: List[str] = [
        saas_type.value for saas_type in SaaSType if is_match(saas_type.value)
    ]
