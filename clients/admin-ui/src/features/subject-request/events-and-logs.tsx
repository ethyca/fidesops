...
const EventsAndLogs = ({subjectRequest}: EventsAndLogsProps) => {
    ...
    const timelineEntries = resultKeys.map((key) => (
        <Box key={key}>
            <Text color='gray.600' fontWeight='500' fontSize='sm'>{key}</Text>
            <Text cursor='pointer' color='complimentary.500' fontWeight='500' fontSize='sm' onClick={() => {
        ...
        <>
            ...
            <Flex>
                <Box id='ActivityTimeline' width='100%'>
                    <Text color='gray.900' fontSize='md' fontWeight='500' mb={1}>Activity Timeline</Text>
                    {timelineEntries}
                </Box>
                <Box id='Event Details' width='100%'>
                    <Text color='gray.900' fontSize='md' fontWeight='500' mb={1}>Event Details</Text>
                    <Box>
                        {eventDetails?.map((detail) => (
                            <Text key={detail.updated_at}>
                                {detail.collection_name} {detail.status} {detail.action_type} {detail.message}
                            </Text>
                        ))}
                    </Box>
                </Box>
