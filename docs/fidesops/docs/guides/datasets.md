...
- `fidesops_meta`: The fidesops\_meta section specifies some additional fields that control how Fidesops manages your data:
	...
	- `data_type` (_Optional_): An indication of type of data held by this field. `data_type` is required only when processing erasure requests for masking strategies other than Null rewrite. This is because Fidesops needs to know the type of data expected by the field in order to generate an appropriate masked value. Available datatypes are `string`, `integer`, `float`, `boolean`, and `object_id`.
	- `length` (_Optional_): An indicator of field length. If `length` is specified, in an erasure request, Fidesops will truncate the end of the masked value to the specified `length`. This should be used if your database column has a length restriction. E.g. if a specific row has `email` of `jerry@mail.com`, and the masked value is `23982r3n8rupq8ewurnw`, and `length` is set to `10`, then the resulting update query will use the value `23982r3n8r`.
