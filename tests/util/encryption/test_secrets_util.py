...
from fidesops.service.masking.strategy.masking_strategy_hmac import HMAC, HmacMaskingStrategy
...
def test_generate_secret() -> None:
    ...
    result: str = SecretsUtil.get_or_generate_secret(None, SecretType.key, masking_meta_key[SecretType.key])
    assert result


def test_build_masking_secrets_for_cache() -> None:
    # build masking secret meta for all HMAC secrets
    masking_meta: Dict[SecretType, MaskingSecretMeta] = HmacMaskingStrategy._build_masking_secret_meta()
