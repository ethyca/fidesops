import pytest
import json
from starlette.status import HTTP_401_UNAUTHORIZED, HTTP_403_FORBIDDEN, HTTP_422_UNPROCESSABLE_ENTITY, HTTP_201_CREATED, \
    HTTP_404_NOT_FOUND, HTTP_200_OK

from fidesops.api.v1.urn_registry import V1_URL_PREFIX, USER_PERMISSIONS
from fidesops.api.v1.scope_registry import USER_PERMISSION_CREATE, USER_PERMISSION_READ, USER_PERMISSION_UPDATE, \
    SAAS_CONFIG_READ, PRIVACY_REQUEST_READ
...
class TestEditUserPermissions:
    ...
    def test_edit_user_permissions_wrong_scope(self, url, api_client, generate_auth_header):
        auth_header = generate_auth_header([SAAS_CONFIG_READ])
    ...
    def test_edit_user_permissions_invalid_user_id(
        ...
        response = api_client.put(f"{V1_URL_PREFIX}/user/{invalid_user_id}/permission", headers=auth_header, json=body)
        permissions = FidesopsUserPermissions.get_by(db, field="user_id", value=invalid_user_id)
        assert HTTP_404_NOT_FOUND == response.status_code
        assert permissions is None
        user.delete(db)
...
class TestGetUserPermissions:
    ...
    def test_get_user_permissions_wrong_scope(self, url, api_client, generate_auth_header):
        auth_header = generate_auth_header([SAAS_CONFIG_READ])
    ...
    def test_get_user_permissions_invalid_user_id(
        ...
        body = {"id": permissions.id, "scopes": [PRIVACY_REQUEST_READ]}
        response = api_client.get(f"{V1_URL_PREFIX}/user/{invalid_user_id}/permission", headers=auth_header, json=body)
        permissions = FidesopsUserPermissions.get_by(db, field="user_id", value=invalid_user_id)
        assert HTTP_404_NOT_FOUND == response.status_code
        assert permissions is None
        user.delete(db)
    ...
    def test_get_user_permissions(
        ...
        response = api_client.get(f"{V1_URL_PREFIX}/user/{user.id}/permission", headers=auth_header)
        response_body = json.loads(response.text)
        assert HTTP_200_OK == response.status_code
        assert response_body['id'] == permissions.id
