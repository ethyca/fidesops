...
def mixpanel_secrets():
    return {
        ...
        "api_secret": pydash.get(saas_config, "mixpanel.api_secret")
        or os.environ.get("MIXPANEL_API_SECRET"),
        "token": pydash.get(saas_config, "mixpanel.token")
        or os.environ.get("MIXPANEL_TOKEN"),
...
def mixpanel_create_test_data(mixpanel_identity_email, mixpanel_connection_config):
    ...
    resp = requests.post(
        f"https://mixpanel.com/api/app/data-deletions/v3.0/?token={mixpanel_secrets['token']}",
        headers={
            **headers,
