...
from fidesops.graph.config import Collection, CollectionAddress, Field, FieldAddress, ScalarField
...
def create_sample_value(field: Field):
    def get_function_name(field) -> str:
        ...
        if field.name in names:
            return field.name
        if isinstance(field, ScalarField) and field.data_type_converter== DataType.integer.value:
            return "integer"
        return "string"
